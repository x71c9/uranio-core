...
function _has_all_properties(atom_name, atom_shape) {
    ...
    for (const [k] of Object.entries(stc_1.atom_common_properties)) {
        if (!atm_util.is_optional_property(atom_name, k) && !urn_lib_1.urn_util.object.has_key(atom_shape, k)) {
            missin_props.push(k);
        }
    }
...
function _has_no_other_properties(atom_name, partial_atom) {
    ...
    for (const k in partial_atom) {
        ...
        if (urn_lib_1.urn_util.object.has_key(stc_1.atom_common_properties, k)) {
            continue;
        }
...
function _validate_hard_properties(molecule) {
    let k;
    for (k in stc_1.atom_hard_properties) {
        try {
            _validate_primitive_type(k, stc_1.atom_hard_properties[k], molecule[k]);
            _validate_custom_type(k, stc_1.atom_hard_properties[k], molecule[k]);
        }
        catch (e) {
            const exc = e;
            if (exc.type === urn_lib_1.urn_exception.ExceptionType.INVALID_ATOM) {
