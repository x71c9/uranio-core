...
const urn_ctx = uranio_utils_1.urn_context.create(default_env_1.core_client_env, is_production(), 'CORE:ENV:CLIENT');
...
function set_client_env() {
    ...
    if (typeof process_log_level === 'string' && process_log_level !== '') {
        if (typeof uranio_utils_1.urn_log.LogLevel[process_log_level] === 'number') {
            env['log_level'] = uranio_utils_1.urn_log.LogLevel[process_log_level];
        }
        else {
