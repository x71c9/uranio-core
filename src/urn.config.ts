...
import {AtomConfig, AtomFieldType} from './types';

type AtomNameUnion = 'media' | 'work';

export const urn_atom_config:AtomConfig<AtomNameUnion>  = {
	atoms: {
		media: {
			filename: {
		...
		work: {
			...
			price: {
				...
				label: 'Price',
				format: {decimal: 2, decimal_point: '.', thousands_step: ','}
			},
			password: {
			...
			confirmation_date: {
				...
				validation: {min: new Date('1970-01-01'), max: new Date('2030-12-31T23:59:59')}
			},
			categories: {
				type: AtomFieldType.SET,
				label: 'Categories',
				required: true,
				values: ['Primary', 'Secondary', 3],
